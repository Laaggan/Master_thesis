...
class Generator2(keras.utils.Sequence):
    ...
    def __init__(self, indices, path_to_folder, batch_size, modality, cropping=True):
        ...
        self.indices = indices
        self.batch_size = batch_size
        self.rand_batch = np.random.choice(indices, 1)
        self.modality = modality
    ...
    def __getitem__(self, index):
        ...
        try:
            slices = np.random.choice(np.arange(x.shape[0]), self.batch_size)
        except:
            slices = np.arange(x.shape[0])

        x = x[slices, :, :, self.modality]
        y = y[slices, :, :, :]
        H = x.shape[1]
        W = x.shape[2]
        x = x.reshape(-1, H, W, 1)
