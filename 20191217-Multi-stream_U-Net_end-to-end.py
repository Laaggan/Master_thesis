...
# Setup the model
unet = multi_stream_unet_end_to_end(input_size=input_size, num_classes=4, lr=lr, metrics=metrics)
...
weights_path = datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + '-all-lr-' + str(lr)\
               + '-n-' + str(X_train.shape[0]) + "-Multi-stream_U-Net_end-to-end.hdf5"
log_dir = "logs/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "-all" \
          + '-lr-' + str(lr) + '-n-' + str(X_train.shape[0]) + '-Multi-stream_U-Net_end-to-end'
